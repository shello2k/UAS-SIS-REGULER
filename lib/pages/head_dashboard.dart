...
class _HeadDashboardState extends State<HeadDashboard> {  
  ...
  return _suratList.where((surat) {  
    ...
    final isNotRejectedOrApproved = surat.status_surat != 'Rejected' && surat.status_surat != 'On Progress'; // Exclude rejected and approved surat  
    return matchesSearch && matchesCategory && isNotRejectedOrApproved; // Add the new condition  
  }).toList();  
}  
          ...
          children: [  
            ...
            Row(  
              children: [  
                CircleAvatar(  
                  radius: 30,  
                  backgroundImage: AssetImage('assets/head_profile.png'), // Static image  
                ),  
                SizedBox(width: 10),  
                Expanded(  
                  child: Row(  
  ...
  void _onItemTapped(int index) {  
    ...
    if (index == 0) {  
      ...
      Navigator.push(  
        context,  
        MaterialPageRoute(  
          builder: (context) => ProfilePage(), // Replace with your actual ProfilePage  
        ),  
      );  
    }  
  }  
