...
class ProfilePage extends StatefulWidget {
  final bool isStudent; // Parameter untuk menentukan apakah pengguna adalah mahasiswa

  ProfilePage({required this.isStudent}); // Constructor untuk menerima parameter
  ...
  Future<void> _fetchUserData() async {
    try {
      ...
      if (currentUser != null) {
        ...
        final userDoc = await _firestore.collection('users').where('email', isEqualTo: currentUser.email).get();

        if (userDoc.docs.isNotEmpty) {
          setState(() {
            userData = userDoc.docs.first.data();
            _isVerified = userData['isVerified'] ?? true; // Mendapatkan status verifikasi dari data
            isLoading = false;
          });
        } else {
          setState(() {
    ...
    return Scaffold(
                  ...
                  Center(
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor: _isVerified
                            ? Colors.green
                            : Colors.red, // Warna tombol berdasarkan status verifikasi
                      ),
                      onPressed: () {
                        // TODO: Implement logic for Firebase Authentication verification
                        setState(() {
                          _isVerified = !_isVerified; // Toggle status verifikasi untuk demo
                        });
                      },
                      child: Text(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (index) {
          ...
          // Navigasi ke halaman yang sesuai
          if (index == 0) {
            Navigator.pushNamed(context, 'student_dashboard'); // Ganti dengan route yang sesuai
          }
        },
      ),
      floatingActionButton:
          widget.isStudent // Tampilkan FAB hanya untuk mahasiswa
              ? FloatingActionButton(
                  onPressed: () {
                    // TODO: Implement logic for creating a new proposal
                    Navigator.pushNamed(context, 'new_request'); // Ganti dengan route untuk pengajuan proposal
                  },
                  child: const Icon(Icons.add),
                  backgroundColor: Colors.orange,
                )
